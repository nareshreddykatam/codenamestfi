...
function broadcastLobbyState(
  ...
  const readiness =
    roomManager.checkReadiness(room);
...
/**
 * Broadcast role-appropriate game state
 * to each individual player.
 *
 * IMPORTANT:
 * The complete secret board is NEVER sent
 * to Operatives/Spectators.
 */
function broadcastGameState(
  ...
  for (const [ws, client] of clients.entries()) {
    ...
    const player =
      room.players.get(client.playerId);

    const isSpymaster =
      player?.role === 'SPYMASTER';

    const maskedState =
      gameSessionManager.getMaskedGameState(
        room,
        player
      );
          ...
          switch (type) {
            case 'CREATE_ROOM': {
              ...
              send(ws, 'ROOM_JOINED', {
                player,
                lobby:
                  roomManager.getLobbyState(
                    room
                  ),
                readiness:
                  roomManager.checkReadiness(
                    room
                  ),
                sessionToken: token,
              });
 ...
 * Listen on 0.0.0.0 rather than localhost.
 */
server.listen(
  PORT,
  HOST,
  () => {
    console.log(
      `[Codenames TFI Server] Running on ${HOST}:${PORT}`
    );
  }
);
